...
class inferenceGPT:
    ...
    def __init__(self):
        ...
        self.manual_annotation        = True                             ## step 6
    ...
    def dpo_finetune_with_kl(self, model, base_model, preference_data, optimizer=None, device="cuda", epochs=3, beta=0.05):
        ...
        if self.freeze_layers:
            model     = self.freeze_gpt_layers(model, freeze_mode="ff_layers", N=2)
        ...
        for epoch in range(epochs):
            ...
            for item in preference_data:
                ...
                pred_new = pred_new_full[:, -9:, 2]    ## [:, -9:, :]

                # Base model prediction
                with torch.no_grad():
                    pred_old_full, _ = base_model.generate(input_tensor, max_new_tokens=9, reasoning_steps=10)
                    pred_old = pred_old_full[:, -9:, 2]    ## [:, -9:, :]

                # Scores: lower MSE means more preferred
                pref_score = F.mse_loss(pred_new, preferred_tensor[:, -9:, 2], reduction='mean')   ## [:, -9:, :]
                rej_score  = F.mse_loss(pred_new,  rejected_tensor[:, -9:, 2],  reduction='mean')
    ...
    def dpo_finetune_with_kl_LAS2(self, model, base_model, preference_data, optimizer=None, device="cuda", epochs=3, beta=0.05):
        ...
        for epoch in range(epochs):
            ...
            for item in preference_data:
                ...
                logp_pref_new = -F.mse_loss(pred_new, preferred_tensor[:, -9:, :], reduction='mean')   ## sum or mean
                logp_rej_new  = -F.mse_loss(pred_new,  rejected_tensor[:, -9:, :], reduction='mean')
                ...
                with torch.no_grad():
                    ...
                    logp_pref_old = -F.mse_loss(pred_old , preferred_tensor[:, -9:, :],  reduction='mean')   ## sum or mean
                    logp_rej_old  = -F.mse_loss(pred_old ,  rejected_tensor[:, -9:, :],  reduction='mean')
                ...
                # DPO loss using softplus (equivalent to cross-entropy for 2 logits)
                ## dpo_loss = F.softplus(logp_rej_new - logp_pref_new)    ## revert back to this 
                
                if self.use_grpo:
                    T = 0.1
                    logits = torch.stack([logp_pref_new / T, logp_rej_new / T]).unsqueeze(0)
                    labels = torch.tensor([0], device=device)
                    preference_loss = F.cross_entropy(logits, labels)           ## GRPO loss
                    beta = 1.0   ## 0.1     ## 0.05
                else:
                    preference_loss = F.softplus(logp_rej_new - logp_pref_new)  ## DPO loss
                    beta = 0.001        ## 0.05
                ...
                MAX_KL_THRESHOLD = 5.0  # or 10
                
                if kl_term.abs() > MAX_KL_THRESHOLD:
                    print(f"[Warning] Large KL term: {kl_term.item():.4f}")
                    # Optional: stabilize KL term if exploding
                    kl_term = kl_term / (1 + kl_term.abs())  # Uncomment to normalize

                #################################################

                # Final combined loss
                total_preference_loss = preference_loss + beta * kl_term

                optimizer.zero_grad()
                total_preference_loss.backward()
                optimizer.step()
                
                print(
                    f"[Epoch {epoch+1}] "
                    f"PrefLoss={preference_loss.item():.4f} | "
                    f"KL={kl_term.item():.4f} | "
                    f"Total={total_loss_step.item():.4f}"
                )

                
                total_loss += total_preference_loss.item()
