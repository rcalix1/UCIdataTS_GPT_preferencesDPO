...
class tsDataWrangling:
    ...
    def read_UCI_data(self):
        self.UCI_Energy_data_df          = pd.read_csv('input/UCI/energydata_complete.csv'   )
        ## self.for_RNN_data_CIVS        = pd.read_csv('input/step1/merged_seq_5998_CIVS.csv')
        ## self.MinByMin_137MB_data_CIVS = pd.read_csv('input/step1/CIVS_137_MB_MinbyMin.csv')
        
    def format_appliances_data_to_match_CIVS(self):
        # Drop the 'date' column
        df = self.UCI_Energy_data_df 
        df = df.drop(columns=["date"])
        # Move 'Appliances' to be the second column (index 2)
        appliances_col = df.pop("Appliances")
        df.insert(2, "Appliances", appliances_col)
        self.UCI_Energy_data_df = df
